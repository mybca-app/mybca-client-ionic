...
export const EventsListEntry: React.FC<EventsListEntryProps> = ({
  event,
  standalone,
}) => {
    ...
    >
      <div style={{ textAlign: "center", paddingRight: "15px" }}>
        <p style={{ marginBlockEnd: "0" }}>
          <IonText>{getShortMonth(date)}</IonText>
        </p>
      ...
      <IonLabel>
        ...
        <div>
          ...
          {event.organization && (
            <IonChip>
              <IonIcon icon={people} />
              <IonLabel>{event.expand["organization"].name}</IonLabel>
            </IonChip>
          )}
        </div>
      </IonLabel>
